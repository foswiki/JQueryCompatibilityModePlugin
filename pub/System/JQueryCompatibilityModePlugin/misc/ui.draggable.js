...
$.widget("ui.draggable", $.extend({}, $.ui.mouse, {
	...
	getHandle: function(e) {
		...
		var handle = !this.options.handle || !$j(this.options.handle, this.element).length ? true : false;
		$j(this.options.handle, this.element)
			.find("*")
			.andSelf()
			.each(function() {
	...
	createHelper: function() {
		...
		var o = this.options;
		var helper = $.isFunction(o.helper) ? $j(o.helper.apply(this.element[0], [e])) : (o.helper == 'clone' ? this.element.clone() : this.element);
	...
	_mouseCapture: function(e) {
		...
		if (this.helper || o.disabled || $j(e.target).is('.ui-resizable-handle'))
			return false;
	...
	setContainment: function() {
		...
		if(o.containment == 'document' || o.containment == 'window') this.containment = [
			...
			$j(o.containment == 'document' ? document : window).width() - this.offset.relative.left - this.offset.parent.left - this.helperProportions.width - this.margins.left - (parseInt(this.element.css("marginRight"),10) || 0),
			($j(o.containment == 'document' ? document : window).height() || document.body.parentNode.scrollHeight) - this.offset.relative.top - this.offset.parent.top - this.helperProportions.height - this.margins.top - (parseInt(this.element.css("marginBottom"),10) || 0)
		];
		
		if(!(/^(document|window|parent)$/).test(o.containment)) {
			var ce = $j(o.containment)[0];
			var co = $j(o.containment).offset();
			var over = ($j(ce).css("overflow") != 'hidden');
			
			this.containment = [
				co.left + (parseInt($j(ce).css("borderLeftWidth"),10) || 0) - this.offset.relative.left - this.offset.parent.left,
				co.top + (parseInt($j(ce).css("borderTopWidth"),10) || 0) - this.offset.relative.top - this.offset.parent.top,
				co.left+(over ? Math.max(ce.scrollWidth,ce.offsetWidth) : ce.offsetWidth) - (parseInt($j(ce).css("borderLeftWidth"),10) || 0) - this.offset.relative.left - this.offset.parent.left - this.helperProportions.width - this.margins.left - (parseInt(this.element.css("marginRight"),10) || 0),
				co.top+(over ? Math.max(ce.scrollHeight,ce.offsetHeight) : ce.offsetHeight) - (parseInt($j(ce).css("borderTopWidth"),10) || 0) - this.offset.relative.top - this.offset.parent.top - this.helperProportions.height - this.margins.top - (parseInt(this.element.css("marginBottom"),10) || 0)
			];
		}
	...
	_convertPositionTo: function(d, pos) {
		...
		return {
			top: (
				...
				+ (this.cssPosition == "fixed" ? $j(document).scrollTop() : 0) * mod
			...
			left: (
				...
				+ (this.cssPosition == "fixed" ? $j(document).scrollLeft() : 0) * mod
	...
	_generatePosition: function(e) {
		...
		var position = {
			top: (
				...
				- (this.cssPosition == "fixed" ? $j(document).scrollTop() : 0)
			),
			left: (
				...
				- (this.cssPosition == "fixed" ? $j(document).scrollLeft() : 0)
			)
		};
	...
	_mouseStop: function(e) {
		...
		if((this.options.revert == "invalid" && !dropped) || (this.options.revert == "valid" && dropped) || this.options.revert === true || ($.isFunction(this.options.revert) && this.options.revert.call(this.element, dropped))) {
			var self = this;
			$j(this.helper).animate(this.originalPosition, parseInt(this.options.revertDuration, 10) || 500, function() {
...
$.ui.plugin.add("draggable", "cursor", {
	...
	stop: function(e, ui) {
		if (ui.options._cursor) $j('body').css("cursor", ui.options._cursor);
	}
});
...
$.ui.plugin.add("draggable", "zIndex", {
	...
	stop: function(e, ui) {
		if(ui.options._zIndex) $j(ui.helper).css('zIndex', ui.options._zIndex);
	}
});
...
$.ui.plugin.add("draggable", "opacity", {
	...
	stop: function(e, ui) {
		if(ui.options._opacity) $j(ui.helper).css('opacity', ui.options._opacity);
	}
});

$.ui.plugin.add("draggable", "iframeFix", {
	start: function(e, ui) {
		$j(ui.options.iframeFix === true ? "iframe" : ui.options.iframeFix).each(function() {					
	...
	stop: function(e, ui) {
		$j("div.ui-draggable-iframeFix").each(function() { this.parentNode.removeChild(this); }); //Remove frame helpers	
	}
});
...
$.ui.plugin.add("draggable", "scroll", {
	...
	drag: function(e, ui) {
		...
		if(i.overflowY[0] != document && i.overflowY[0].tagName != 'HTML') {
			...
			if(e.pageY - $j(document).scrollTop() < o.scrollSensitivity)
				scrolled = $j(document).scrollTop($j(document).scrollTop() - o.scrollSpeed);
			if($j(window).height() - (e.pageY - $j(document).scrollTop()) < o.scrollSensitivity)
				scrolled = $j(document).scrollTop($j(document).scrollTop() + o.scrollSpeed);
		}
		...
		if(i.overflowX[0] != document && i.overflowX[0].tagName != 'HTML') {
			...
			if(e.pageX - $j(document).scrollLeft() < o.scrollSensitivity)
				scrolled = $j(document).scrollLeft($j(document).scrollLeft() - o.scrollSpeed);
			if($j(window).width() - (e.pageX - $j(document).scrollLeft()) < o.scrollSensitivity)
				scrolled = $j(document).scrollLeft($j(document).scrollLeft() + o.scrollSpeed);
		}
...
$.ui.plugin.add("draggable", "snap", {
	start: function(e, ui) {
		...
		var inst = $j(this).data("draggable");
		inst.snapElements = [];

		$j(ui.options.snap.constructor != String ? ( ui.options.snap.items || ':data(draggable)' ) : ui.options.snap).each(function() {
			var $t = $j(this); var $o = $t.offset();
...
$.ui.plugin.add("draggable", "connectToSortable", {
	start: function(e,ui) {
		...
		var inst = $j(this).data("draggable");
		inst.sortables = [];
		$j(ui.options.connectToSortable).each(function() {
	...
	drag: function(e,ui) {
		...
		var inst = $j(this).data("draggable"), self = this;
		...
		$.each(inst.sortables, function(i) {
			...
			if(checkPos.call(inst, this.instance.containerCache)) {
				...
				if(!this.instance.isOver) {
					...
					this.instance.currentItem = $j(self).clone().appendTo(this.instance.element).data("sortable-item", true);
...
$.ui.plugin.add("draggable", "stack", {
	start: function(e,ui) {
		var group = $.makeArray($j(ui.options.stack.group)).sort(function(a,b) {
			return (parseInt($j(a).css("zIndex"),10) || ui.options.stack.min) - (parseInt($j(b).css("zIndex"),10) || ui.options.stack.min);
		});
